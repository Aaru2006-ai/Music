...
def queue_kb(chat_id: int, page: int, total_pages: int) -> InlineKeyboardMarkup:
    nav = []
    if page > 1:
        nav.append(InlineKeyboardButton("◀️ Prev", callback_data=f"queue:{chat_id}:{page - 1}"))
    if page < total_pages:
        nav.append(InlineKeyboardButton("Next ▶️", callback_data=f"queue:{chat_id}:{page + 1}"))

    rows = []
    if nav:
        rows.append(nav)
    rows.append([
        InlineKeyboardButton("🗑 Clear Queue", callback_data=f"clear_queue:{chat_id}"),
        InlineKeyboardButton("🔀 Shuffle",     callback_data=f"shuffle:{chat_id}"),
    ])
...
def playlist_kb(playlists: list, owner_id: int) -> InlineKeyboardMarkup:
    ...
    for pl in playlists:
        ...
        buttons.append([
            InlineKeyboardButton(f"📂 {name}", callback_data=f"load_playlist:{owner_id}:{name}"),
            InlineKeyboardButton("🗑", callback_data=f"del_playlist:{owner_id}:{name}"),
        ])
...
def settings_kb(chat_id: int, settings: dict) -> InlineKeyboardMarkup:
    ...
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton(f"🔁 Loop: {loop_icon}",       callback_data=f"setting:loop:{chat_id}"),
            InlineKeyboardButton(f"🔀 Shuffle: {shuffle_icon}", callback_data=f"setting:shuffle:{chat_id}"),
        ],
        [
            InlineKeyboardButton(f"▶️ Autoplay: {auto_icon}",   callback_data=f"setting:autoplay:{chat_id}"),
            InlineKeyboardButton(f"🛡 Admin Only: {admin_icon}", callback_data=f"setting:admin_only:{chat_id}"),
        ],
        [
            InlineKeyboardButton("🔊 Quality: Audio", callback_data=f"setting:quality:{chat_id}"),
        ],
        [InlineKeyboardButton("❌ Close", callback_data="close")],
    ])
...
def help_kb() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton("🎵 Music",  callback_data="help:music"),
...
def confirm_kb(action: str, identifier: str) -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup([
        [
            InlineKeyboardButton("✅ Confirm", callback_data=f"confirm:{action}:{identifier}"),
